...
int is_chain(shell_info_t *shell_info, char *buffer, size_t *position)
{
    ...
    if (buffer[j] == '|' && buffer[j + 1] == '|')
    {
        buffer[j] = 0;
        j++;
        shell_info->cmd_buffer_type = CMD_OR;
    }
    else if (buffer[j] == '&' && buffer[j + 1] == '&')
    {
        buffer[j] = 0;
        j++;
        shell_info->cmd_buffer_type = CMD_AND;
    }
    else if (buffer[j] == ';') /* found end of this command */
    {
        buffer[j] = 0; /* replace semicolon with null */
        shell_info->cmd_buffer_type = CMD_CHAIN;
    }
    else
        return (0);
    *position = j;
...
void check_chain(shell_info_t *shell_info, char *buffer, size_t *position, size_t start_position, size_t length)
{
    ...
    if (shell_info->cmd_buffer_type == CMD_AND)
    {
    ...
    if (shell_info->cmd_buffer_type == CMD_OR)
    {
        if (!shell_info->status)
        {
            buffer[start_position] = 0;
...
int replace_vars(shell_info_t *shell_info)
{
    ...
    for (i = 0; shell_info->argv[i]; i++)
    {
        ...
        {
            replace_string(&(shell_info->argv[i]), _strdup(convert_number(shell_info->status, 10, 0)));
            continue;
        }
        if (!_strcmp(shell_info->argv[i], "$$"))
        {
            replace_string(&(shell_info->argv[i]), _strdup(convert_number(getpid(), 10, 0)));
            continue;
        }
        node = node_starts_with(shell_info->env, &shell_info->argv[i][1], '=');
        if (node)
        {
            replace_string(&(shell_info->argv[i]), _strdup(_strchr(node->str, '=') + 1));
            continue;
        }
